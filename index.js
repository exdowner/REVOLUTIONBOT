...
client.commands.set('autorole', {
  execute: async (interaction) => {
    ...
    // Simples implementação - salva em variável global
...
client.on('messageCreate', async (message) => {
  ...
  if (cmd === 'serverinfo') {
    const embed = new EmbedBuilder().setTitle('📊 INFO DO SERVIDOR').setColor('#0099ff')
      .addFields(
        ...
        { name: '📅 Criado', value: new Date(message.guild.createdAt).toLocaleDateString('pt-BR'), inline: true }
      ).setTimestamp();
    await message.reply({ embeds: [embed] });
  }
  ...
  if (cmd === 'userinfo') {
    ...
    const embed = new EmbedBuilder().setTitle(`👤 INFO DE ${user.tag}`).setThumbnail(user.displayAvatarURL()).setColor('#0099ff')
      .addFields(
        { name: '🆔 ID', value: user.id, inline: true },
        { name: '📅 Entrou', value: member ? new Date(member.joinedAt).toLocaleDateString('pt-BR') : 'N/A', inline: true }
      ).setTimestamp();
    await message.reply({ embeds: [embed] });
  }
